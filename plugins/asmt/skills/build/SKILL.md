...
3. Read `<paths.changes>/<change-id>/spec.md` and `tasks.md`. If `tasks.md`
   does not exist, stop and run `/asmt:plan` first. Do not invent a task
   breakdown and start building from it.
...
6. Ask `asmt-plan next <paths.changes>/<change-id>` which task is open, and
   take exactly that one. Not two. Not "while I was here". If it names
   nothing, the plan is finished — go to step 10.
...
9. Once the commit exists, run `asmt-plan complete <change-dir> <T-n>`. Do not
   tick the box by hand: the mark means a commit landed, and a command that
   records it is a fact where a claim to have recorded it is not. Then report
   the task ID, the files touched, the test names added, and anything you
   noticed but did not fix — never success without the actual test output.
   Return to step 6.
10. When the plan is finished, run `/asmt:verify` and stop. Opening the PR is a
