...
- **Size the change into a lane** - Fast / Standard / Deep. Don't over-process a small fix or
...
- **Archive on merge:** `/opsx:archive` or `openspec archive <change-id>` folds the delta into
  the living specs under `openspec/specs/`.
- **Never send client/production data through an agent** - source code only. Secrets are
  referenced by environment-variable name and resolved server-side, never inlined; sensitive
  files are blocked at the tool level in `.claude/settings.json`.
<!-- asmt:end -->
